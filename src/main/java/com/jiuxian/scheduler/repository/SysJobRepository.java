...
@Repository
@Transactional
public interface SysJobRepository extends JpaRepository<SysJob, Serializable> {
    List<SysJob> findByEnableStatus(String enableStatus);

    @Modifying
    @Query(" UPDATE SysJob job SET job.jobStatus =:jobStatus  where job.id =:jobId")
    void doUpdateJobStatus(@Param("jobId") Integer jobId, @Param("jobStatus") String jobStatus);
}
